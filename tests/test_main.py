...
@click.option('--sort', type=click.Choice(['name', 'size', 'date']),
              default='name', help='Sort the files by name, size, or date')
...
@click.option('-y', '--yes', is_flag=True, help='Get all files in the current directory without prompting')
...
def csv2md(csv_file, csv_headers, output_dir, create_directories, verbose, very_verbose):
    """
...
class TestCSV2MD:
    @pytest.mark.parametrize('dir_path, filename, sort', [
    ...
    @pytest.mark.parametrize('file_name, title, author, yes', [
    ...
    @pytest.mark.parametrize('csv_file, csv_headers, output_dir', [
        ('data.csv', True, '.'),
        ('data.csv', False, 'output')
    ])
